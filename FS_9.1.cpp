...
static void* handleHttpClient(void* arg) {
    ...
    if (isRootRequest) {
        std::string html =
            ...
            "<div id='panel' style='position:absolute;top:8px;right:8px;z-index:10;background:rgba(0,0,0,0.55);padding:6px;border:1px solid #888;border-radius:8px;max-width:45%;box-sizing:border-box'>"
            ...
            "<div style='position:absolute;bottom:8px;left:8px;z-index:10;user-select:none'>"
            ...
            "var sx=Math.round((e.clientX-r.left)/r.width*1600);"
            "var sy=Math.round((e.clientY-r.top)/r.height*900);"
            "if(sx<0||sy<0||sx>=1600||sy>=900)return;"
