...
static void* handleHttpClient(void* arg) {
    ...
    if (isRootRequest) {
        std::string html =
            ...
            "<img src='/stream.mjpg' style='max-width:100%;max-height:85vh'>"
            "<div id='status' style='color:#0f0;font:18px monospace;margin-top:8px'>"
            "Click here, then use keys</div>"
            "<div style='color:#0ff;font:20px monospace;margin-top:6px'>"
            ...
            "    e.preventDefault();"
            "    var cmd=(k=='escape')?'q':k;"
            "    var st=document.getElementById('status');"
            "    st.textContent='Sending: '+cmd.toUpperCase()+'...';"
            "    st.style.color='#ff0';"
            "    fetch('/cmd/'+cmd).then(function(r){"
            "      st.textContent='Sent: '+cmd.toUpperCase()+' OK';"
            "      st.style.color='#0f0';"
            "    }).catch(function(e){"
