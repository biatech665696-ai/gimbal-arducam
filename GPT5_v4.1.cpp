...
public:
    ...
    Detection detect(cv::Mat &roi, int ox, int oy, double /*learningRate*/ = 0.0)
    {
        ...
        cv::Mat lastAffine;  // prev→curr affine transform
        ...
        cv::Mat fgMask;
        mog2_->apply(stableGray, fgMask, lr);
        ...
        // === FG PIXEL GATE (soft — no frame skipping) ===
        // With stabilization, high FG is rare. If it still happens (e.g. very
        // large motion or auto-exposure), we just suppress detection output
        // but do NOT skip the frame — MOG2 still learns from it.
        bool fgSuppressed = false;
        if (rawFG > 4000) {
            // Extreme noise: don't trust contours this frame
            fgSuppressed = true;
            consecutiveDetections_ = 0;
        }
        ...
        // If FG suppressed: skip contour analysis, return no detection
        // but MOG2 has already processed the frame (no blind gap)
        if (fgSuppressed) {
            lastRawContours_ = 0;
            return d;
        }
