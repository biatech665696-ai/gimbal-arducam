...
void trackingThread(SafeQueue<FrameData>&queue,atomic<bool>&run)
{
    ...
    while(run)
    {
        ...
        if (d.valid && !prevGrayFull.empty()
            ...
            // Skip filter when already tracking: servo follows object → object is
            // nearly stationary in frame → motionMag≈0 → filter kills valid detections.
            // Only apply on first acquisition (consecutiveValid==0).
