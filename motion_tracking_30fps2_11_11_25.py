...
SPEED_THRESHOLD_MPS = 10.0  # порог в м/с для подсветки (только объекты > 100 м/с)
MATCH_MAX_DISTANCE_PX = 20 #200  # макс расстояние для сопоставления между кадрами (не используется в SORT-like трекере)
...
fgbg = cv2.createBackgroundSubtractorKNN(history=100, dist2Threshold=100.0, detectShadows=False)
...
while True:
    ...
    for t in tracks:
        ...
        # регистрируем/показываем только объекты с speed > 100 м/с
