...
# Initialize tracker after class definition
...
# Try to open camera with error handling for memory/pipeline issues
...
# Try to set resolution and FPS, but check for errors
if not cap.set(cv2.CAP_PROP_FRAME_WIDTH, FRAME_WIDTH):
    print(f"[WARN] Could not set frame width to {FRAME_WIDTH}. Using default.")
if not cap.set(cv2.CAP_PROP_FRAME_HEIGHT, FRAME_HEIGHT):
    print(f"[WARN] Could not set frame height to {FRAME_HEIGHT}. Using default.")
if not cap.set(cv2.CAP_PROP_FPS, FPS):
    print(f"[WARN] Could not set FPS to {FPS}. Using default.")
...
if actual_fps <= 0 or actual_fps > 1000:
    # Try to estimate FPS if not reported
    print("[WARN] Camera FPS not reported. Estimating FPS...")
    import timeit
    num_frames = 60
    ...
    elapsed = end - start
    estimated_fps = num_frames / elapsed if elapsed > 0 else 0
    print(f"[INFO] Opened camera at index {CAM_INDEX} with resolution {int(actual_width)}x{int(actual_height)} @ ~{estimated_fps:.2f} FPS (estimated)")
else:
